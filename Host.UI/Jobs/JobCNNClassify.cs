        ...
        public JobCNNClassify(GRasterLayer featureRasterLayer, int epochs, int model, int width, int height, int channel, string sampleFilename)
        {
            _t = new Thread(() => {
                using (StreamReader sr = new StreamReader(sampleFilename))
                {
                    ...
                    //create cnn model
                    Summary = "模型训练中";
                    CNN cnn = new CNN(new int[] { channel, width, height }, keys.Count);
                    ...
                    for (int i = 0; i < epochs; i++)
                    {
                        ...
                        for (int k = 0; k < batchSize; k++)
                        {
                            ...
                            inputX[k] = inputList[index].ToArray();
                            //NP.ToOneHot(Array.IndexOf(RandomSeedKeys, rawValueIndex), ActionNum);
                            labels[k] = NP.ToOneHot(Array.IndexOf(keys.ToArray(), outputList[index]), keys.Count);
                        }
